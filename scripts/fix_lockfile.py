...
def update_file(fn, install_repository_dependencies, install_resolver_dependencies):
    ...
    for tool in unlocked['tools']:
        ...
        # over directly, without a reivision. update_tool.py script will fix that.
        if len(locked) == 0:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-norep', '--no-install-repository-dependencies', action='store_true', default=False, help="Don't install TS repository dependencies")
    parser.add_argument('-noresdep', '--no-install-resolver-dependencies', action='store_true', default=True, help="Don't install dependencies via Galaxy dependency resolver (e.g. Conda)")
    parser.add_argument('-resdep', '--install-resolver-dependencies', action='store_false', dest='no_install_resolver_dependencies', help="Install dependencies via Galaxy dependency resolver (e.g. Conda)")
